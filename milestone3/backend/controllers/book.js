...
async function getFilterInfo(req, res) {
  ...
  solr
    ...
    .then(function (resp) {
      let solrResp;
      if (field === "genre")
        solrResp = resp.data.facet_counts.facet_fields.genre;
...
async function getSearchResult(req, res) {
  ...
  if (req.query.weights === undefined || req.query.weights === [])
    params.append("qf", fields.join(" "));
  else {
  ...
  params.append(
    "fq",
    `rating:[${req.query.rating[0]} TO ${req.query.rating[1]}]`
  );
  ...
  solr
    ...
    .then(function (resp) {
      ...
      return res
        .status(200)
        .send({ books: result, numFound: resp.data.response.numFound });
    })
    .catch((error) => {
      console.log(error);
      return res.status(400).json("Something went wrong!");
